...
import { appendMarkdownContinuation, getMarkdownTextStats, hasMeaningfulMarkdownContent, markdownToPlainText } from "@/lib/chapters/markdown";
import { Button } from "@/shared/ui/button";
import { Card } from "@/shared/ui/card";
import { Input } from "@/shared/ui/input";
import { AISuggestionPanel, fetchAI, useAICooldown } from "@/widgets/story-editor/ai-client";
import { MarkdownChapterEditor } from "@/widgets/story-editor/markdown-chapter-editor";
...
export function ChapterDraftsEditor({
  ...
  const removeChapter = (index: number) => {
    ...
    if ((chapter.title.trim() || hasMeaningfulMarkdownContent(chapter.content)) && !window.confirm(`Удалить главу «${chapter.title || index + 1}»?`)) return;
  ...
  const continueChapter = async (chapter: ChapterDraft) => {
    ...
    try {
      const result = await fetchAI({
        ...
        title: storyTitle,
        description,
        chapterText: markdownToPlainText(chapter.content)
      });
  ...
  return (
    <div className="min-w-0 space-y-5">
      {chapters.map((chapter, index) => {
        const textStats = getMarkdownTextStats(chapter.content);
        ...
        return (
          ...
          <div className="mb-5 flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 truncate text-sm text-text-muted">
                {textStats.characters.toLocaleString("ru-RU")} символов · {textStats.words.toLocaleString("ru-RU")} слов
              </p>
            </div>
          ...
          <div className="min-w-0 space-y-4">
            ...
            <MarkdownChapterEditor
              error={errors[chapter.clientId]}
              onChange={(content) => updateChapter(chapter.clientId, { content })}
            ...
            {continuations[chapter.clientId] ? (
              <AISuggestionPanel actionLabel="Вставить продолжение" onApply={() => {
                updateChapter(chapter.clientId, { content: appendMarkdownContinuation(chapter.content, continuations[chapter.clientId]) });
